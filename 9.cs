...
namespace Zadacha9
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Расчёт момента инерции системы относительно оси Z, проходящей через ЦМС
            for (int i = 0; i < m.Length; i++)
            {
                inert_z = m[i] * ((X_center - x[i]) * (X_center - x[i]) + m[i] * ((Y_center - y[i]) * (Y_center - y[i])));
            }
            Console.WriteLine("Момент инерции системы относительно оси Z: " + inert_z.ToString());
        }
    }
}